...
| 字段 | 类型 | 必填 | 说明 |
|-----|------|------|------|
| mainAccountId | String | 是 | 所属主账户ID |
| tradeAccount | String | 是 | 交易账户（业务唯一标识） |
| symbol | String | 是 | 交易对(如BTCUSDT) |
| orderType | Enum(OrderType) | 是 | 枚举值：LIMIT(限价)、MARKET(市价) |
| side | Enum(OrderSide) | 是 | 枚举值：BUY(买入)、SELL(卖出) |
| price | String | 否 | 价格(市价单可不传，限价单必填) |
| quantity | String | 是 | 数量 |
| clientOrderId | String | 否 | 客户端订单ID(可选) |

补充说明：
- `orderType = LIMIT` 时必须传 `price`
- `orderType = MARKET` 时 `price` 可不传
...
```json
{
  ...
  "timestamp": 1704528000000,
  "data": "{\"mainAccountId\":\"main001\",\"tradeAccount\":\"trade001\",\"symbol\":\"BTCUSDT\",\"orderType\":\"LIMIT\",\"side\":\"BUY\",\"price\":\"40000.00\",\"quantity\":\"1.5\",\"clientOrderId\":\"client_order_123\"}"
}
```
...
```json
{
  ...
  "data": "{\"orderId\":\"ORD1704528000000_abc123\",\"tradeAccount\":\"trade001\"}"
}
```
...
```json
{
  ...
  "orderId": "ORD1704528000000_abc123",
  "timestamp": 1704528000000,
  "data": "{\"orderId\":\"ORD1704528000000_abc123\",\"mainAccountId\":\"main001\",\"tradeAccount\":\"trade001\",\"symbol\":\"BTCUSDT\",\"orderType\":\"LIMIT\",\"side\":\"BUY\",\"price\":\"40000.00\",\"quantity\":\"1.5\",\"filledQty\":\"0\",\"avgPrice\":\"0\",\"status\":\"SUBMITTED\",\"createTime\":1704528000000,\"updateTime\":1704528000000,\"clientOrderId\":\"client_order_123\"}"
}
```

**data字段中的Order对象包含**:
- orderId: 订单ID
- mainAccountId: 所属主账户ID
- tradeAccount: 交易账户（业务唯一标识）
- symbol: 交易对
...
**完整ZeroMQ消息示例**（带"ORDER."主题前缀）:
```
ORDER.{"eventType":"ORDER_CANCEL","orderId":"ORD1704528000000_abc123","timestamp":1704528100000,"data":"{\"orderId\":\"ORD1704528000000_abc123\"}"}
```
...
```json
{
  ...
  "orderId": "ORD1704528000000_abc123",
  "timestamp": 1704528100000,
  "data": "{\"orderId\":\"ORD1704528000000_abc123\"}"
}
```
...
| maker | Boolean | 是 | 是否为Maker(挂单方) |

说明：EMS回传Trade时不需要再传用户字段，Trading Order会根据`orderId`回填`mainAccountId`和`tradeAccount`。
...
### 6.1 C++网关连接Trading Order（网关端伪代码）

```cpp
// 1. 作为TCP客户端连接Trading Order
...
while (true) {
    ...
    // 4. 解析消息（格式："ORDER." + EmsMessage JSON）
    string messageWithTopic(static_cast<char*>(message.data()), message.size());

    // 5. 去掉topic前缀
    string topic = "ORDER.";
    if (messageWithTopic.substr(0, topic.length()) != topic) {
        continue; // 跳过不匹配的消息
    }
    string jsonStr = messageWithTopic.substr(topic.length());

    // 6. 解析EmsMessage
    json emsMessage = json::parse(jsonStr);
    string eventType = emsMessage["eventType"];
    string orderId = emsMessage["orderId"];

    if (eventType == "ORDER_SUBMIT") {
        // 解析Order对象
        json order = json::parse(emsMessage["data"].get<string>());
...
### 6.3 EMS发送成交通知（EMS端伪代码）

```cpp
// 1. 创建ZeroMQ PUB Socket连接5556端口
zmq::context_t context(1);
...
json trade = {
    ...
    {"fee", "0.0015"},
    {"feeAsset", "BTC"},
    {"tradeTime", getCurrentTimeMillis()},
    {"maker", false}
};

// 3. 序列化Trade对象
string tradeJson = trade.dump();

// 4. 添加topic前缀并发送（格式："TRADE." + Trade JSON）
string topic = "TRADE.";
string messageWithTopic = topic + tradeJson;

zmq::message_t message(messageWithTopic.begin(), messageWithTopic.end());
...
1. **TCP连接管理**: 作为TCP客户端，连接到Trading Order的TCP服务器（127.0.0.1:9900）
2. **消息幂等性**: 使用msgId进行消息去重
3. **粘包处理**: 必须按Length-Prefix协议解析
4. **订单转发**: 接收用户HTTP订单后，构造ORDER_REQUEST发送给Trading Order
5. **同步响应**: 等待Trading Order的ORDER_RESPONSE后再返回给用户
6. **错误处理**: 返回明确的错误码和错误信息给用户
7. **连接保活**: 建议使用TCP keepalive机制
8. **重连机制**: 连接断开后应自动重连

### 7.2 EMS系统开发注意事项

1. **双向通信（PUB-SUB模式）**:
   - 使用SUB Socket，绑定到5555端口接收订单事件
   - 使用PUB Socket，连接到5556端口发送成交通知
2. **订阅主题**:
   - SUB Socket必须订阅"ORDER."主题来接收订单事件
   - 如果需要接收所有消息，可以订阅空字符串 `socket.subscribe("")`
3. **Topic处理**:
   - 接收消息时，去掉"ORDER."前缀后再解析JSON
   - 发送消息时，在Trade JSON前添加"TRADE."前缀
4. **消息格式**:
   - 接收格式：`"ORDER." + EmsMessage JSON`
   - 发送格式：`"TRADE." + Trade JSON`
5. **订单事件处理**:
   - 收到ORDER_SUBMIT后，将订单加入撮合队列
   - 收到ORDER_CANCEL后，从撮合队列移除订单
6. **撮合逻辑**: 实现订单撮合算法，匹配买卖双方
7. **成交通知**:
   - 撮合成功后，推送双边成交通知(买方和卖方)到Trading Order
   - 每条消息格式："TRADE." + Trade对象JSON
8. **消息顺序**: 按timestamp处理事件顺序
9. **幂等性**: 同一个订单可能收到多次事件，需要处理重复消息
10. **慢订阅者问题**: PUB-SUB模式中，如果订阅者处理慢，可能丢失消息，需设置合适的HWM(高水位标记)

### 7.3 Trading Order系统说明

- **TCP服务器**: 自动启动，监听9900端口
- **ZeroMQ（PUB-SUB模式）**:
  - PUB Socket自动连接5555端口，发送带"ORDER."主题前缀的订单事件
  - SUB Socket自动绑定5556端口，订阅"TRADE."主题接收成交通知
- **数据存储**: 订单、成交、资产数据存储在Redis
- **交易对信息**: 从币安/欧易API获取并缓存到Redis

---

## 8. 系统配置

### 8.1 Trading Order配置(application.yml)
...
zeromq:
  ems:
    endpoint: "tcp://127.0.0.1:5555"  # 发送订单事件到EMS
  trade:
    port: 5556  # 接收EMS成交通知
```
...
| 端口 | 协议 | 角色 | Topic | 用途 |
|-----|------|------|-------|------|
| 9900 | TCP | Trading Order(服务器) | N/A | 接收网关的订单请求 |
| 5555 | ZeroMQ PUB-SUB | EMS(SUB绑定) | ORDER. | 接收Trading Order的订单事件 |
| 5556 | ZeroMQ PUB-SUB | Trading Order(SUB绑定) | TRADE. | 接收EMS的成交通知 |
